...
✅ Define constants: BASE_URL, AUTH_URL
✅ Use environment variables: process.env.BASE_URL || 'default_value'

⚠️ SYNTAX & STRUCTURE:
✅ Use ONLY ${framework} syntax (NO mixing frameworks)
✅ Proper bracket matching and indentation
✅ ONE main test suite wrapping all endpoints
✅ Use const for all declarations, NOT let or var

⚠️ TEST ORGANIZATION:
✅ Clear, unique endpoint descriptions
✅ Logical grouping by endpoint
✅ No duplicate describe blocks
✅ ALL test functions at the same nesting level within their endpoint suite
...
${options.includeErrorScenarios ? `2. ❌ ERROR TESTS: 400, 401, 403, 404, 422, 500
   ${method !== 'GET' ? '- Test with invalid content-type headers' : ''}` : ''}
${options.includeEdgeCases ? `3. 🔧 EDGE CASES: Empty/null values, boundary values, special chars
   ${isGraphQL ? '- GraphQL-specific edge cases (query depth, invalid syntax)' : ''}` : ''}
${options.includeSecurityTests ? `4. 🛡️ SECURITY: XSS injection, SQL injection, auth bypass` : ''}
...
❌ Contains "continue", "add more", "follow pattern", or "similar tests"
...
⚡ GENERATE COMPLETE TESTS FOR ALL ${uniqueEndpoints.size} ENDPOINTS NOW - NO EXCEPTIONS:`;
  ...
  private generateAuthenticationSection(authFlow: AuthFlow, framework: string, customAuthGuide?: string): string {
    ...
    authSection += `1. ✅ Generate auth setup using ${authFlow.authPattern} pattern\n`;
    authSection += `2. ✅ Extract tokens from login response and chain to subsequent requests\n`;
    authSection += `3. ✅ Use environment variables for credentials\n`;
    authSection += `4. ✅ Include beforeAll setup for authentication state\n`;
    authSection += `5. ✅ Generate auth error tests (401, 403)\n`;
    authSection += `6. ✅ Include proper cleanup in afterAll hooks\n`;

    if (customAuthGuide && customAuthGuide.trim()) {
      authSection += `\n🎯 CUSTOM AUTH GUIDE (HIGHEST PRIORITY):\n${customAuthGuide.trim()}\n`;
      authSection += `🚨 Follow the custom guide above EXACTLY.\n`;
    }
